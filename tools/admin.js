...
function keywordValues(id){return [...new Set($(id).value.split(/[,\n]/).map((value)=>value.trim()).filter(Boolean))]}
function updateKeywordResult(){setResult("keywordResult",`공식자료 ${keywordValues("keywordText").length}개 · 뉴스 ${keywordValues("newsKeywordText").length}개 키워드가 입력되어 있습니다.`)}
$("keywordText").addEventListener("input",updateKeywordResult);
$("newsKeywordText").addEventListener("input",updateKeywordResult);
$("downloadNewsKeywords").addEventListener("click",()=>{const words=keywordValues("newsKeywordText");if(!words.length){setResult("keywordResult","뉴스 키워드를 한 개 이상 입력하세요.","error");return}download("news_keywords.txt",`${words.join("\n")}\n`,"text/plain;charset=utf-8");setResult("keywordResult",`뉴스 전용 키워드 ${words.length}개를 news_keywords.txt로 만들었습니다.`,"success")});
$("downloadSettings").addEventListener("click",()=>{const words=keywordValues("keywordText"),newsWords=keywordValues("newsKeywordText");if(!words.length){setResult("keywordResult","공식자료 키워드를 한 개 이상 입력하세요.","error");return}const sources=[];if($("sourcePress").checked)sources.push({name:"기후부 보도자료",url:"https://www.mcee.go.kr/home/web/board/rss.do?menuId=286&boardMasterId=1"});if($("sourceNotice").checked)sources.push({name:"기후부 공지·공고",url:"https://www.mcee.go.kr/home/web/board/rss.do?menuId=290&boardMasterId=39"});if($("sourceNews").checked&&!newsWords.length){setResult("keywordResult","뉴스를 수집하려면 뉴스 전용 키워드를 한 개 이상 입력하세요.","error");return}const news=$("sourceNews").checked?[{name:"배출권시장 뉴스",query:"config/news_keywords.txt의 키워드를 사용"}]:[];if(!sources.length&&!news.length){setResult("keywordResult","수집할 자료를 한 개 이상 선택하세요.","error");return}download("settings.json",`${JSON.stringify({policyKeywords:words,newsKeywordFile:"config/news_keywords.txt",policySources:sources,policyNewsSearches:news,policyLookbackDays:30,maxNewsItems:24,maxPolicyItems:60},null,2)}\n`,"application/json");setResult("keywordResult","공식자료와 뉴스 키워드를 분리한 settings.json을 만들었습니다.","success")});
